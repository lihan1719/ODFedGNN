...
class MyLoss(nn.Module):
    def __init__(self, gamma1=200, gamma2=1):
    ...
    def forward(self, y1, y2, y_true):
        # 计算 loss_val
        loss_val = nn.MSELoss()(y1, y_true)

        # 计算 loss_prob
        mask = y_true >= 1
        mask = mask.float()
        # loss_prob = -1 / (y_true.shape[0]**2) * (
        #     (1 - mask) * self.gamma2 * y2.pow(2) * torch.log(1 - y2) +
        #     mask * self.gamma1 * (1 - y2).pow(2) * torch.log(y2)).sum()
        loss_prob = -(
            (1 - mask) * self.gamma2 * y2.pow(2) * torch.log(1 - y2) +
            mask * self.gamma1 * (1 - y2).pow(2) * torch.log(y2)).mean()
        # 计算总的 loss
        loss = loss_val + loss_prob
...
class ODFedNodePredictorClient(nn.Module):
    def __init__(self, base_model_name, optimizer_name, train_dataset,
                 ...
                 lr, weight_decay, batch_size, client_device,
        ...
        with torch.enable_grad():
            for epoch_i in range(self.sync_every_n_epoch):
                ...
                for batch in self.train_dataloader:
                    ...
                    metrics = unscaled_metrics(y_pred_val, y,
    ...
    def local_eval(self, dataloader, name, state_dict_to_load):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                y_pred_val, y_pred_prob = self(data, server_graph_encoding)
                if name == 'test':
                    od_prediction.append(y_pred_val.cpu().detach().numpy())
                # loss = nn.MSELoss()(y_pred, y)
                loss = MyLoss()(y_pred_val, y_pred_prob, y)
                num_samples += x.shape[0]
                metrics = unscaled_metrics(y_pred_val, y, self.feature_scaler,
                                           name)
...
class ODFedNodePredictor(LightningModule):
            ...
            for epoch_i in range(self.hparams.server_epoch + 1):
                ...
                for batch in server_train_dataloader:
                    ...
                    if epoch_i == self.hparams.server_epoch:
                        ...
                        y_pred_val, y_pred_prob = self.base_model.forward_decoder(
                            data,
                            h_encode,
        ...
        if self.hparams.mp_worker_num <= 1:
            ...
            for worker_i, client_params in enumerate(
                ...
                local_train_results.append(
                    pool.apply_async(
                        ODFedNodePredictorClient.client_local_execute,
                        args=(device_name,
                ...
                if selected is not None:
                    ...
                    if is_trans:
                        if k == 'num_samples':
                            ...
                            agg_log_t[
                                k] += local_log[k] * local_log['num_samples']
                    else:
                        if k == 'num_samples':
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.hparams.mp_worker_num <= 1:
            for client_i, client_params in enumerate(self.client_params_list):
                local_val_result = ODFedNodePredictorClient.client_local_execute(
                    batch.device, deepcopy(self.base_model.state_dict()),
                    'test', client_params)
                y_pred = local_val_result[0]['od_prediction']
                local_val_result[0].pop('od_prediction')
        ...
        np.save(
            ...
            '_od_prediction.npy', od_prediction)
        # 2. aggregate
