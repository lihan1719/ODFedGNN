...
class SplitFedHeteroNodePredictorClient(nn.Module):
    def __init__(self, base_model_name, optimizer_name, train_dataset,
                 val_dataset, test_dataset, feature_scaler, sync_every_n_epoch,
                 lr, weight_decay, mtl_lambda, batch_size, client_device,
                 start_global_step, *args, **kwargs):
        super().__init__()
    ...
    def local_train(self, state_dict_to_load):
        ...
        neighbor_flattened_params_list = [
            self._get_flattened_model_params(x.items()).to(self.device)
            for x in state_dict_list[1:]
        ]
        self.train()
        with torch.enable_grad():
            for epoch_i in range(self.sync_every_n_epoch):
                ...
                for batch in self.train_dataloader:
                    ...
                    mtl_loss = 0
                    self_flattend_params = self._get_flattened_model_params(
                        self.base_model.named_parameters())
                    for weight_j, flattened_params_j in zip(
                            neighbor_weight_list,
                            neighbor_flattened_params_list):
                        mtl_loss += (weight_j *
                                     (self_flattend_params -
                                      flattened_params_j).norm('fro'))
                    ...
                    if type(mtl_loss) is not int:
                        epoch_log['train/mtl_loss'] += mtl_loss.detach(
                        ) * x.shape[0]
                    else:
    ...
    def client_local_execute(device, order, hparams_list):
        ...
        for hparams in hparams_list:
            ...
            client = SplitFedHeteroNodePredictorClient(client_device=device,
                                                       **hparams)
            if order == 'train':
...
class SplitFedHeteroNodePredictor(LightningModule):
    ...
    def setup(self, step):
        ...
        if self.identical_agg_model:
            self.base_model = getattr(base_models,
            ...
            for idx in range(num_clients):
                self.base_model.append(
                    getattr(base_models, self.hparams.base_model_name)(
                        input_size=input_size,
                        output_size=output_size,
                        **self.hparams))
                self.base_model[-1].load_state_dict(
                    deepcopy(self.base_model[0].state_dict()))
            self.base_model = nn.ModuleList(self.base_model)

        if self.fmtl:
            # preprocess edge_index, edge_attr to lists of neighbor weights
            edge_index, edge_attr = self.data['train'][
                'edge_index'], self.data['train']['edge_attr']
            ...
            for eid, (s, t) in enumerate(edge_index.transpose()):
                if s != t:
                    self.lists_of_neighbors[s].append(
                        (edge_attr[eid].item(), t))
        ...
        if self.fmtl:
            for n_w, n_id in self.lists_of_neighbors[idx]:
                weighted_neighbors.append(
                    (n_w, copied_model_state_dicts[n_id]))
                ...
                for batch in server_train_dataloader:
                    ...
                    if self.identical_agg_model:
                        ...
                        h_encode = self.base_model.forward_encoder(
                            data).detach()  # L x (B x N) x F
                    else:
                        h_encode = []
                        for client_i, client_model in enumerate(
                                self.base_model):
                            data = dict(
                            ...
                            h_encode_i = client_model.forward_encoder(
                                data).detach()
                    ...
                    if epoch_i == self.hparams.server_epoch:
                        ...
                        if self.identical_agg_model:
                            ...
                            h_encode = h_encode.view(h_encode.shape[0],
                                                     batch_num, node_num,
                                                     h_encode.shape[2])
                            ...
                            for client_i, client_model in enumerate(
                                ...
                                y_pred = client_model.forward_decoder(
                                    data,
                                    h_encode[:, :,
                                             client_i:client_i + 1, :].flatten(
                                                 1, 2),
                                    batches_seen=global_step,
                                    return_encoding=False,
                                    server_graph_encoding=
                                    graph_encoding_to_clients[
                                        client_i:client_i + 1])
            ...
            for batch in server_dataloader:
                ...
                if self.identical_agg_model:
                    ...
                    for client_i, client_model in enumerate(self.base_model):
                        data = dict(x=x[:, :, client_i:client_i + 1, :],
                                    x_attr=x_attr[:, :,
                                                  client_i:client_i + 1, :],
                                    y=y[:, :, client_i:client_i + 1, :],
                                    y_attr=y_attr[:, :,
    ...
    def training_step(self, batch, batch_idx):
        ...
        for client_i, client_params in enumerate(self.client_params_list):
            copied_model_state_dicts.append(
                self._get_copied_ith_model(client_i))
        # fill neighbors
        for client_i, client_params in enumerate(self.client_params_list):
            client_params.update(
                state_dict_to_load=self._get_weighted_neighboring_models(
                    client_i, copied_model_state_dicts))

        if self.hparams.mp_worker_num <= 1:
            for client_i, client_params in enumerate(self.client_params_list):
                local_train_result = SplitFedHeteroNodePredictorClient.client_local_execute(
                    batch.device, 'train', **client_params)
            ...
            for worker_i, client_params in enumerate(
                ...
                local_train_results.append(
                    pool.apply_async(
                        SplitFedHeteroNodePredictorClient.client_local_execute,
                        args=(device_name, 'train', client_params)))
            pool.close()
            pool.join()
        ...
        if self.identical_agg_model:
            ...
            for idx in range(len(self.base_model)):
                self.base_model[idx].load_state_dict(
                    agg_local_train_results['state_dict'][idx])
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for client_i, client_params in enumerate(self.client_params_list):
            client_params.update(
                state_dict_to_load=self._get_copied_ith_model(client_i))
        if self.hparams.mp_worker_num <= 1:
            for client_i, client_params in enumerate(self.client_params_list):
                local_val_result = SplitFedHeteroNodePredictorClient.client_local_execute(
                    batch.device, 'val', **client_params)
            ...
            for worker_i, client_params in enumerate(
                ...
                local_val_results.append(
                    pool.apply_async(
                        SplitFedHeteroNodePredictorClient.client_local_execute,
                        args=(device_name, 'val', client_params)))
            pool.close()
            pool.join()
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.hparams.mp_worker_num <= 1:
            for client_i, client_params in enumerate(self.client_params_list):
                local_val_result = SplitFedHeteroNodePredictorClient.client_local_execute(
                    batch.device, 'test', **client_params)
                local_val_results.append(local_val_result)
        else:
            pool = mp.Pool(self.hparams.mp_worker_num)
            for worker_i, client_params in enumerate(
                ...
                local_val_results.append(
                    pool.apply_async(
                        SplitFedHeteroNodePredictorClient.client_local_execute,
                        args=(device_name, 'test', client_params)))
            pool.close()
            pool.join()
            local_val_results = list(map(lambda x: x.get(), local_val_results))
            local_val_results = list(
