...
from models.base_models.GraphNets_v1 import GraphNet
...
class SplitGNNNodePredictorClient(nn.Module):
    def __init__(self, optimizer_name, input_size, hidden_size, output_size,
                 train_dataset, val_dataset, test_dataset, feature_scaler, lr,
                 weight_decay, batch_size, *args, **kwargs):
        super().__init__()
        ...
        self.base_model = GRUSeq2SeqWithGraphNet(input_size=input_size,
    ...
    def forward(self, x, stage):
        if stage == 'enc':
            ...
            return self.base_model.forward_decoder(data, h_encode,
                                                   batches_seen, False,
                                                   server_graph_encoding)
        else:
        ...
        self.encoding = self(
            {
                ...
                'x_attr': x_attr,
                'y_attr': y_attr
            }, stage='enc')  # L x (B x N) x F
        return self.encoding

    def local_decode_forward(self, server_graph_encoding, batches_seen):
        self.server_graph_encoding = server_graph_encoding.clone().detach(
        ).requires_grad_(True)
        ...
        self.y_pred = self(
            {
                ...
                'server_graph_encoding': self.server_graph_encoding
            },
            stage='dec')
    ...
    def local_backward(self, grads=None, stage='dec'):
        if stage == 'dec':
            ...
            epoch_log.update(**unscaled_metrics(self.y_pred, self.y,
                                                self.feature_scaler, 'train'))
            return {'log': epoch_log, 'grad': self.server_graph_encoding.grad}
        elif stage == 'enc':
    ...
    def local_eval(self, name):
        ...
        epoch_log.update(
            **unscaled_metrics(self.y_pred, self.y, self.feature_scaler, name))
...
class SplitGNNNodePredictor(LightningModule):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--no_agg_on_client_models',
                            dest='agg_on_client_models',
                            action='store_false')
    ...
    def setup(self, step):
        ...
        shuffled_train_idx = shufflerng.permutation(train_sample_num).astype(
            int)
        ...
        if self.hparams.hetero_graph:
            for name in ['edge_index', 'edge_attr']:
                data['train'][name] = [
                    data['train'][name][tid] for tid in shuffled_train_idx
                ]

        self.graph_dataset = {}
        for name in ['train', 'val', 'test']:
            if self.hparams.hetero_graph:
                self.graph_dataset[name] = EdgeDataset(
                    self.data[name]['edge_index'],
                    self.data[name]['edge_attr'])
            else:
                self.graph_dataset[name] = TensorDataset(
                    torch.zeros_like(
                        self.data[name]
                        ['y']))  # fake dataset, used for 1 batch forward only
        ...
        self.prop_model_optimizer = torch.optim.Adam(
            self.prop_model.parameters(),
            lr=self.hparams.lr,
            weight_decay=self.hparams.weight_decay)

    def train_dataloader(self):
        # return a fake dataloader for running the loop
        return DataLoader(self.graph_dataset['train'],
                          batch_size=self.hparams.batch_size,
                          collate_fn=EdgeDataset.collate_fn)

    def val_dataloader(self):
        return DataLoader(self.graph_dataset['val'],
                          batch_size=self.hparams.batch_size,
                          collate_fn=EdgeDataset.collate_fn)

    def test_dataloader(self):
        return DataLoader(self.graph_dataset['test'],
                          batch_size=self.hparams.batch_size,
                          collate_fn=EdgeDataset.collate_fn)
    ...
    def training_step(self, batch, batch_idx):
        ...
        stacked_encodings = stacked_encodings.permute(2, 1, 0,
                                                      3)  # N x B x L x F
        if self.hparams.hetero_graph:
            to_batch_graph_list = []
            for bi in range(stacked_encodings['x'].shape[0]):
                to_batch_graph_list.append(
                    Data(x=stacked_encodings,
                         edge_attr=edge_attr_batch[bi],
                         edge_index=edge_index_batch[bi]))
            ...
            data = Data(x=stacked_encodings,
                        edge_index=self.data['train']['edge_index'].to(
                            stacked_encodings.device),
        ...
        if self.hparams.hetero_graph:
            hiddens = hiddens.view(len(to_batch_graph_list), -1,
                                   hiddens.shape[1], hiddens.shape[2]).permute(
                                       0, 2, 1, 3)  # N x B x L x F
        # 3. run decoding on all clients
        for client_i, client in enumerate(self.clients):
            client.local_decode_forward(hiddens[client_i:client_i + 1],
                                        batches_seen)  # 1 x B x L x F
        ...
        if self.hparams.hetero_graph:
            ...
            for name in [
                ...
                if data[name].grad is not None:
                    data_grads_all[name] = data[name].grad.view(
                        -1, len(self.clients), data[name].shape[1],
                        data[name].shape[2]).permute(0, 2, 1, 3)
                else:
            ...
            for client_i, client in enumerate(self.clients):
                ...
                for name in [
                    ...
                    if data_grads_all[name] is not None:
                        data_grads[name] = data_grads_all[name][
                            client_i:client_i + 1]  # 1 x B x L x F
                    else:
            ...
            for client_i, client in enumerate(self.clients):
                ...
                for name in [
                    ...
                    if data[name].grad is not None:
                        data_grads[name] = data[name].grad[client_i:client_i +
                                                           1]  # 1 x B x L x F
                        data_grads[name] = data_grads[name].permute(
                            2, 1, 0, 3).flatten(1, 2)  # L x (B x 1) x F
                    else:
                        data_grads[name] = None
                client.local_backward(grads=data_grads['x'], stage='enc')
