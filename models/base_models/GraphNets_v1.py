...
class EdgeModel(nn.Module):
    def __init__(self, node_input_size, edge_input_size, global_input_size,
        ...
        self.edge_mlp = MLP_GN(edge_mlp_input_size, hidden_size,
                               edge_output_size, 2, activation, dropout)
...
class NodeModel(nn.Module):
    def __init__(self, node_input_size, edge_input_size, global_input_size,
        ...
        self.node_mlp = MLP_GN(node_mlp_input_size, hidden_size,
                               node_output_size, 2, activation, dropout)
...
class GlobalModel(nn.Module):
    def __init__(self, node_input_size, edge_input_size, global_input_size,
        ...
        self.global_mlp = MLP_GN(global_mlp_input_size, hidden_size,
                                 global_output_size, 2, activation, dropout)
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        agg_edge = scatter_add(scatter_add(edge_attr,
                                           col,
                                           dim=0,
                                           dim_size=x.size(0)),
                               batch,
...
class GraphNet(nn.Module):
    def __init__(self, node_input_size, edge_input_size, global_input_size,
                 hidden_size, updated_node_size, updated_edge_size,
                 updated_global_size, node_output_size, gn_layer_num,
                 activation, dropout, *args, **kwargs):
        super().__init__()

        self.global_input_size = global_input_size

        self.net = []
        last_node_input_size = node_input_size  # node_input、global input\node output 都是hidden_size
        last_edge_input_size = edge_input_size
        last_global_input_size = global_input_size
        for _ in range(gn_layer_num):
            edge_model = EdgeModel(last_node_input_size, last_edge_input_size,
                                   last_global_input_size, hidden_size,
                                   updated_edge_size, activation, dropout)
            last_edge_input_size += updated_edge_size
            node_model = NodeModel(last_node_input_size, updated_edge_size,
                                   last_global_input_size, hidden_size,
                                   updated_node_size, activation, dropout)
            last_node_input_size += updated_node_size
            global_model = GlobalModel(updated_node_size, updated_edge_size,
                                       last_global_input_size, hidden_size,
                                       updated_global_size, activation,
                                       dropout)
    ...
    def forward(self, data):
        ...
        edge_attr = edge_attr.expand(-1, x.shape[1], x.shape[2], -1, -1)
        u = x.new_zeros(*([batch[-1] + 1] + list(x.shape[1:-1]) +
                          [self.global_input_size]))
        for net in self.net:
            updated_x, updated_edge_attr, updated_u = net(
