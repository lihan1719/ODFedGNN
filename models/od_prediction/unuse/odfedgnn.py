    ...
    def local_eval(self, dataloader, name, state_dict_to_load):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                y_pred = self(data, server_graph_encoding)
                if name == 'test':
                    od_prediction.append(y_pred.cpu().detach().numpy())
                loss = nn.MSELoss()(y_pred, y)
                num_samples += x.shape[0]
                metrics = unscaled_metrics(y_pred, y, self.feature_scaler,
                                           name)
    ...
    def client_local_execute(device, state_dict_to_load, order, hparams_list):
        ...
        if type(hparams_list) == 'list':
            for hparams in hparams_list:
                client = ODClient(client_device=device, **hparams)
                if order == 'train':
            ...
            client = ODClient(client_device=device, **hparams_list)
            if order == 'train':
                res = client.local_train(state_dict_to_load)
            elif order == 'val':
                res = client.local_validation(state_dict_to_load)
...
class ODServer(LightningModule):
