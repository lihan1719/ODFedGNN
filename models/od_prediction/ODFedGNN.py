...
from models.od_prediction.NYC_Centralized import unscaled_metrics
        ...
        with torch.enable_grad():
            for epoch_i in range(self.sync_every_n_epoch):
                ...
                for batch in self.train_dataloader:
                    ...
                    loss = WMSELoss(self.feature_scaler, 10000)(y_pred, y)
    ...
    def local_eval(self, dataloader, name, state_dict_to_load):
        ...
        with torch.no_grad():
            ...
            for batch in dataloader:
                ...
                y_pred = self(data, server_graph_encoding)
                if name == 'test':
                    od_prediction.append(y_pred.detach().cpu())
                # loss = nn.MSELoss()(y_pred, y)
                loss = WMSELoss(self.feature_scaler, 10000)(y_pred, y)
                num_samples += x.shape[0]
...
class ODFedNodePredictor(LightningModule):
    ...
    def setup(self, step):
        ...
        self.gcn = GraphNet(node_input_size=self.hparams.hidden_size,
                            edge_input_size=1,
            ...
            for epoch_i in range(self.hparams.server_epoch + 1):
                ...
                for batch in server_train_dataloader:
                    ...
                    if epoch_i == self.hparams.server_epoch:
                        ...
                        loss = WMSELoss(self.data['feature_scaler'],
                                        10000)(y_pred, y)
                        # loss = nn.MSELoss()(y_pred, y)
    ...
    def test_step(self, batch, batch_idx):
        ...
        od_groundtruth = self.data['test']['y'].detach().cpu().numpy()
        ...
        np.save(
            ...
            'od_prediction.npy', od_prediction)
        np.save(
            self.hparams.output_dir + '/' + self.hparams['model_name'] + '/' +
            'od_groundtruth.npy', od_groundtruth)
        del log['num_samples']
        with open(
                self.hparams.output_dir + '/' + self.hparams['model_name'] +
