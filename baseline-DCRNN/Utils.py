...
class DataInput(object):
    ...
    def load_data(self):
        ...
        data = np.load(self.data_dir +
                       '/NYC_BOD_2019-07-2019-07_Grid300m_60T.npy')
        data = data[..., :-3]

        # 初始化缩放器
        mean = np.mean(data)
        std = np.std(data)
        scaler = Z_score_scaler(mean=mean, std=std)

        ODdata = scaler.transform(data)
        print(ODdata.shape)

        adj = np.load(self.data_dir + '/Geo_adj_Grid300m.npy')
...
class DataGenerator(object):
    ...
    def split2len(self, data_len: int):
        mode_len = dict()
        mode_len['train'] = int(self.data_split_ratio[0] /
                                sum(self.data_split_ratio) * data_len)
        mode_len['validate'] = int(self.data_split_ratio[1] /
                                   sum(self.data_split_ratio) * data_len)
    ...
    def get_data_loader(self, data: dict, params: dict):
        ...
        feat_dict = dict()
        feat_dict['x_seq'] = torch.from_numpy(np.asarray(x_seq)).float().to(
            params['GPU'])
        ...
        for mode in ['train', 'validate', 'test']:
            dataset = ODDataset(inputs=feat_dict,
                                output=y_seq,
                                mode=mode,
                                mode_len=mode_len)
            print('Here is data loader', mode, dataset.inputs['x_seq'].shape,
                  dataset.output.shape)
            data_loader[mode] = DataLoader(dataset=dataset,
...
class ODDataset(Dataset):
    def __init__(self, inputs: dict, output: torch.Tensor, mode: str,
                 mode_len: dict):
        self.mode = mode
    ...
    def prepare_xy(self, inputs: dict, output: torch.Tensor):
        ...
        x = dict()
        x['x_seq'] = inputs['x_seq'][start_idx:(start_idx +
                                                self.mode_len[self.mode])]
...
class AdjProcessor():
    ...
    def process(self, adj: torch.Tensor):
        ...
        if self.kernel_type in ['localpool', 'chebyshev']:  # spectral
            ...
            if self.kernel_type == 'localpool':
                localpool = torch.eye(
                    adj_norm.shape[0]
                ) + adj_norm  # same as add self-loop first
            ...
            kernel_list += forward_series + backward_series[
                1:]  # 0-order Chebyshev polynomial is same: I
    ...
    def rescale_laplacian(L):
        ...
        try:
            lambda_ = torch.eig(L)[0][:,
                                      0]  # get the real parts of eigenvalues
