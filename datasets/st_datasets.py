...
def load_pems_data(rootpath,
    ...
    if ratio < 1.0:
        inductive = True
        partial_nodes = np.load(os.path.join(
            rootpath, 'sensor_graph', '{}_partial_nodes.npz'.format(name)),
                                allow_pickle=True)
    ...
    feature_scaler = StandardScaler(mean=train_features.mean(axis=0),
    ...
    for name in ['train', 'val', 'test']:
        ...
        x_attr = attr_scaler.transform(
            raw_data[name]['x'][..., ATTR_START:ATTR_END])
        y_attr = attr_scaler.transform(
            raw_data[name]['y'][..., ATTR_START:ATTR_END])
...
available_datasets = {
    'METR-LA':
    partial(load_pems_data,
            rootpath='data/traffic/data',
            name='METR-LA',
            adj_mx_name='adj_mx.pkl'),
    'METR-LA-onenode':
    partial(load_pems_data,
            rootpath='data/traffic/data',
            name='METR-LA',
    ...
    partial(load_pems_data,
            ...
            ratio=0.9),
    'NRI_springs5':
    partial(load_hetero_nri_sim_data, rootpath='data/NRI', name='springs5')
}
...
def load_nyc_data(path, data_name):
    ...
    # feature_scaler = StandardScaler(mean=train_features.mean(axis=0),
    #                                 std=train_features.std(axis=0))
    attr_scaler = StandardScaler(mean=attr_features.mean(axis=0),
                                 std=attr_features.std(axis=0))
    feature_scaler = None
    loaded_data = {
        'feature_scaler': feature_scaler,
        'attr_scaler': attr_scaler
...
def load_dataset(dataset_name, data_dir, data_name):
    if dataset_name == 'NYC_bike':
        loaded_data = load_nyc_data(path=data_dir, data_name=data_name)
        return loaded_data
